  ...
  vector[n] mu_new;
  {
    ...
    //real K2sum;
    {
      ...
      matrix[N2, N1] Y = to_matrix(y, N2, N1);
      {
        ...
        matrix[N2, N2] K2 = Gram_fBM(X2,Hurst2);
        K2rowsum =  K2 * rep_vector(1,N2);
        //K2sum = sum(K2rowsum);
        K2c = Gram_centring(K2);
        K2 = Gram_square(K2c);
        {
    ...
    /// posterior predective mean and variance
    {
...

      {
        ...
        for (i in 1:n2){
          ...
          K2_trnew_c[,i] = kvec_cen(kvec_fBM(X2, to_vector(x2_new[i,]), Hurst2) ,K2rowsum);
          for (j in 1:i){
            K2_new[i,j] = 1 + square(alpha2)*dot_product(K2_trnew_c[,i], K2_trnew_c[,j]);
            K2_new[j,i] = K2_new[i,j];
          }
        }
        K2_trnew = rep_matrix(1,N2,n2) + square(alpha2) * (K2c*K2_trnew_c);
      }
      B1 = Q1'*K1_trnew;
      C1 = Q1'*RE_trnew;
      B2 = Q2'*K2_trnew;
