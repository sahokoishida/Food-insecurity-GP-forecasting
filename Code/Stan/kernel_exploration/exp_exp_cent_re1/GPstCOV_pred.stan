...
transformed data {
  ...
  vector[n] mu_new;
  {
    ...
    //real K2sum;
    {
      ...
      m = to_vector(Q2' * (Y * Q1));
      h = to_vector(Q2' * (to_matrix(D*beta, N2, N1) * Q1));
    // computing eigenvalues of the model matrix K + sigma^2*I
     ...
     mloglik = -0.5 * sum(square(m-h)./eval) - 0.5 * sum(log(eval)) - 0.5 *log(2*pi());
    ...
    /// posterior predective mean and variance
    {
...

      {
        vector[n1] k1tmp;
        vector[n2] k2tmp;
        //matrix[N2,n2] K2_trnew_c;
        //matrix[N2,n2] K2_trnew_tmp;
        for (i in 1:n1){
        ...
        for (i in 1:n2){
          //K2_trnew_tmp[,i] = kvec_fBM(X2, to_vector(x2_new[i,]), Hurst2);
          K2_trnew[,i] = kvec_exponential(X2, to_vector(x2_new[i,]), rho2);
          k2tmp[i] = sum(K2_trnew[,i]);
          for (j in 1:i){
            K2_new[i,j] =  1 + square(alpha2)*(kernel_exponential(to_vector(x2_new[i,]),to_vector(x2_new[j,]),rho2) - k2tmp[i]/n2 - k2tmp[j]/n2 +sum(K2rowsum)/square(n2));
      ...
      // mean
      mu_new = (D_new*beta)+square(alpha0)*to_vector(B2' * (to_matrix(q, N2, N1)*B1)) + square(sigma1)*to_vector(C2' * (to_matrix(q, N2, N1)*C1));
      // variance
